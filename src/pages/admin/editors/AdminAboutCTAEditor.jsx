...
import {
  getAboutCtaAdmin,
  subscribeToAboutCtaChanges,
  saveAboutCta,
} from '../../../api/aboutctaadmin'; 
...
const AdminAboutCTAEditor = () => {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    const data = await getAboutCtaAdmin();
    if (data) {
  ...
  useEffect(() => {
    const channel = subscribeToAboutCtaChanges(loadData); // Re-run loadData on any change
    return () => {
  ...
  const onSubmit = async (data) => {
    ...
    const { success, error } = await saveAboutCta(data); 
    ...
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      <header className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">About Page CTA Editor</h1>
        </div>
      </header>
            ...
            <div className="bg-white rounded-lg shadow-md border border-gray-200">
              <div className="p-6 space-y-6">
                <p className="text-sm text-gray-600">
                  Edit the content for the call-to-action block at the bottom of the About page.
                </p>
                <FormInput 
                  label="Title" 
                  name="title" 
...
export default AdminAboutCTAEditor;