...
/**
 * Fetches all feature showcase items for the admin panel.
 * @returns {Array} - A list of item objects.
 */
export async function getFeatureShowcaseAdmin() {
  try {
    ...
    console.error('Error fetching admin feature items:', error.message);
    return [];
  }
}

/**
 * Subscribes to changes on feature_showcase_items and media_assets tables.
 * @param {function} callback - The function to call on any change.
 * @returns {object} - The subscription channel.
 */
export const subscribeToFeatureShowcaseChanges = (callback) => {
  const tables = ['feature_showcase_items', 'media_assets'];
...
/**
 * Updates the 'order' field for a list of items.
 * @param {Array} items - The array of item objects in their new order.
 * @returns {object} - { success: true } or { success: false, error }
 */
export async function updateFeatureShowcaseOrder(items) {
  try {
    ...
    const { error } = await supabase.from('feature_showcase_items').upsert(updates);
    if (error) throw error;
    return { success: true };
  } catch (error) {
    console.error('Error updating item order:', error.message);
...
/**
 * Deletes a feature item and its associated media asset (if any).
 * @param {object} item - The item object to delete.
 * @returns {object} - { success: true } or { success: false, error }
 */
export async function deleteFeatureShowcaseItem(item) {
  try {
    // 1. Delete the feature_showcase_items row
 ...
 */
export async function saveFeatureShowcaseItem(itemData, userId) {
  if (!userId) {
  ...
  try {
    ...
    const { itemToSave, mediaIdsToDelete } = await processItemUpload(itemData, userId);

    // 2. Upsert the feature_showcase_items record
...
/**
 * =================================================================
 * Helper Functions (Reused from Logo Cloud)
 * =================================================================
 */

/**
 * Handles uploading a new item image file if one is present.
 * @param {object} itemData - The form data.
 * @param {string} userId - The user ID.
 * @returns {object} - { itemToSave, mediaIdsToDelete }
 */
async function processItemUpload(itemData, userId) {
