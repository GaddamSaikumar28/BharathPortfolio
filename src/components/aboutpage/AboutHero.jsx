...
const AboutHero = () => {
  ...
  // --- FIX 1: Safely access file_path ---
  // We use optional chaining (?.) so it doesn't crash if media_assets is null
  const filePath = hero.media_assets?.file_path;
  
  // --- FIX 2: Generate URL correctly ---
  // Only generate the URL if we actually have a file path
  const imageUrl = filePath ? getStorageUrl('public/'+filePath) : null;
  ...
  return (
    <div className="min-h-[90vh] flex items-center bg-white pt-20">
        ...
        >
          ...
          >
            ...
            {imageUrl ? (
              <img
                src={imageUrl}
                alt={hero.media_assets?.alt_text || 'About Hero Image'}
                className="rounded-3xl shadow-2xl w-full max-w-md object-cover aspect-[4/5]"
              />
            ) : (
              // Optional: Placeholder if no image exists in DB
              <div className="rounded-3xl bg-gray-100 w-full max-w-md aspect-[4/5] flex items-center justify-center text-gray-400">
                No Image
              </div>
            )}
          </motion.div>
